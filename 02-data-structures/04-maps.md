...
│  │  │   5     │  3   │  0  │ 0xabc.. │ 0x123 │ ───► bucket array   │
│  │  └─────────┴──────┴─────┴─────────┴──────┘                      │
│  └──────────────────────────────────────────┘                      │
│                              │                                      │
│                              ▼                                      │
...
│  │                                                               │   │
│  │  bucket[0]   bucket[1]   bucket[2]   bucket[3]  ... bucket[7] │   │
│  │  ┌─────┐     ┌─────┐     ┌─────┐     ┌─────┐       ┌─────┐     │   │
│  │  │     │     │ "a" │     │     │     │"d"  │       │     │     │   │
│  │  │ 10  │     │ 100 │     │ 20  │     │ 400 │       │ 50  │     │   │
│  │  │     │     ├─────┤     │     ├─────┤       ├─────┤     │     │
...
│  │     │                                                    │       │   │
│  │     └────────────────────────────────────────────────────┘       │   │
│  │                         (some buckets may have overflow)        │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────┐
│           BUCKET HEADER             │
│  tophash[0] = h[key1] & 0xFF        │
│  tophash[1] = h[key2] & 0xFF        │
│  ...                                 │
│  overflow ──► next bucket (if full)  │
├─────────────────────────────────────┤
│           DATA (8 slots)             │
│                                      │
│  ┌───────────┬───────────┬─────────┐ │
...
Step 2: Use top 8 bits for bucket index
        topbits = 0x6E (in range 0-7)
        bucket = bucket[topbits] = bucket[1]

Step 3: Search bucket
        bucket[1] contains:
        - key="a", value=100  → h1 doesn't match topbits
        - key="b", value=200  → MATCH! return 200 ✓

Step 4: If not found in bucket, follow overflow chain
        until found or chain ends → return zero value
```

### Visual: Map Growth (Rehashing)

```
INITIAL (B=2, 4 buckets, load ~6.5):
...
m["c"] = 300  → bucket 2
m["d"] = 400  → bucket 3
m["e"] = 500  → bucket 0 (collision)
m["f"] = 600  → bucket 1 (collision)
m["g"] = 700  → bucket 2 (collision)
...

BEFORE GROWTH:
┌────────────────────────────────┐
│  Old buckets:  [4]           │
│  count: 27 (6.75 per bucket)  │
│  load > threshold!           │
└────────────────────────────────┘
       │
       ▼ (grow)
...
│  count: 27                     │
│  countPerBucket: ~3.4         │
│  new entries go to new buckets  │
│  old buckets migrate lazily    │
└────────────────────────────────┘
```
