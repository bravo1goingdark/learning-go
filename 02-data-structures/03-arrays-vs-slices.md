...
```
+-------------------+
| Pointer to array  | ──→ [actual data in memory]
+-------------------+
| Length (int)      |     len(s)
+-------------------+
| Capacity (int)    |     cap(s)
...
│            ptr ──┘                                                     │
│                                                                  │
│  len = 5 (accessible: indices 0-4)                                  │
│  cap = 9 (total allocated: indices 0-8)                              │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
AFTER append (space available):
┌────────────┐           ┌────────────────┐
│ s          │           │ [0]  10        │
│ ptr ───────┼──────────►│ [1]  20        │
│ len = 4    │           │ [2]  30        │◄── Written here
│ cap = 3    │           │ [3]  40        │     (no new allocation)
└────────────┘           └────────────────┘
```
...
```
BEFORE (capacity FULL):
┌────────────┐           ┌────────────────┐
│ s          │           │ OLD ARRAY      │
│ ptr ───────┼──────────►│ [0]  10  ←─┐   │
│ len = 3    │           │ [1]  20  ←─┼───┼── Copy to new array
│ cap = 3    │           │ [2]  30  ←─┘   │
└────────────┘           └────────────────┘
...
src:  [1] [2] [3] [4] [5]   (len=5)
           ┌───┐
dst:      [0] [0] [0]       (len=3, cap=3)
           └───┘
            copy 3 elements → dst[0]=1, dst[1]=2, dst[2]=3
...
+-------------------+
| slice variable    |
| Data ─────────────┼──→ Heap: [underlying array]
| Len = 3           |     +---+---+---+---+---+
| Cap = 5           |     | 1 | 2 | 3 |   |   |
+-------------------+     +---+---+---+---+---+
                                    ↑
                            Only elements 0-2 are accessible
```
